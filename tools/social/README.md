...
| `npm run deals` | `marketing/deals-this-week.jpg` + `deals-caption.txt` (1080×1080) | "Longest since a restock" share image, ranked by the same `/cheap` logic as the bot. It carries **no prices** — the ranking is days-since-restock, and per-kg rates differ per store and move with the exchange rate, so a figure baked into a JPEG would be wrong somewhere the day it posts. **JPEG** — Instagram's API accepts nothing else. The caption is written alongside it so the post describes that week's actual ranking. |
...
| `npm run restock-tomorrow` | *(prints a message)* | **The daily "who restocks tomorrow" line** for the Telegram channel. Text, not an image — the value is that it is short and arrives the evening before, so a rendered card would add a Chromium download and a commit for nothing. Reads only the two forward-looking signals the bot's `todayText()` uses (`restockDay`, `restockDates`); never `cycle`, because a cycle says a restock is *due*, not that it happens, and never `restock_date`, which records the last one. Prints nothing when tomorrow has none on record — no store has a Sunday restock day, so Saturdays are silent. `RESTOCK_OFFSET_DAYS=n` shifts the day for testing. |
...
The channel also carries one thing Instagram does not: the daily restock line,
posted by [`.github/workflows/restock-tomorrow.yml`](../../.github/workflows/restock-tomorrow.yml).
